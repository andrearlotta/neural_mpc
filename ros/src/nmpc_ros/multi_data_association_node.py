...
class DataAssociationNode:
    def __init__(self):
        ...
        detection_sub = message_filters.Subscriber("/agent_1/yolov7/detect", Detection2DArray)
        depth_image_sub = message_filters.Subscriber("/agent_1/camera/depth/image/compressed", CompressedImage)

        self.ts = message_filters.ApproximateTimeSynchronizer([detection_sub, depth_image_sub], queue_size=10, slop=0.2)
        self.ts.registerCallback(self.synchronized_callback)

        self.camera_info_sub = rospy.Subscriber("/agent_1/camera/depth/camera_info", CameraInfo, self.camera_info_callback)
