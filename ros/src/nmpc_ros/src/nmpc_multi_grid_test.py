...
# ---------------------------
    def mpc_opt(self, g_nn_raw, g_nn_ripe, trees, lb, ub, x0, lambda_vals, steps=10):
        ...
        # Valid cells (1: assigned, 0: unassigned)
        grid = np.zeros((5, 5))
        # Reshape self.trees_pos in una matrice quadrata 3x3 di coppie
        matrix_size = int(np.sqrt(len(self.trees_pos)))
        centers = self.trees_pos.reshape(matrix_size, matrix_size, 2)
        ...
        for i in range(grid.shape[0]):  # iterazione sulle righe
            for j in range(grid.shape[1]):
                if grid[i,j] == 0:
                    obj += 10*self.penalty_2d(self.current_state[0], self.current_state[1], p=10, s=1)
